  ...
  return (
    <>
      ...
      <Carousel
        ...
        align="start"
        withControls={
          (data?.length ?? 0) > 5 && isSmallerThanTable ? true : false
        }
        slidesToScroll={2}
        // classNames={classes}
        ...
        {loading
          ? [1, 2, 3, 4, 5]?.map((trend) => (
              <Carousel.Slide key={trend}>
                <Skeleton h={300} w={"100%"} className="skeleton" />
                <Skeleton mt={20} h={20} w={"90%"} className="skeleton" />
                <Skeleton mt={20} h={20} w={"50%"} className="skeleton" />
              </Carousel.Slide>
            ))
          : data?.map((trend) => {
              if (trend.id) {
