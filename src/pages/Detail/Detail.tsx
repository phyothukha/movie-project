...
import {
  ...
  useGetMediaCredits,
  useGetSimilarMedia,
  useGetRecommendedMedia,
} from "@/store/server/mediaDetail/queries";
...
const Detail = () => {
  ...
  const { data: detailvideo } = useGetMediaVideos(mediatype, id);
  const { data: credits, isFetching: creditloading } = useGetMediaCredits(
    mediatype,
    id
  );
  ...
  } = useGetSimilarMedia(mediatype, id);
  ...
  } = useGetRecommendedMedia(mediatype, id);
